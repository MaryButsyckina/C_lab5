...
namespace device
{
	...
	public:
		SmartPhone(string brand, string model,
			...
			OS OS = None, string OS_ver = "0",
			int apps = 0, int used_rom = 0);
		...
		int GetApps() const { return apps_; }
		IMEI GetIMEI() const { return IMEI_; }
		float GetUsedROM() const { return used_ROM_; };

		int SetOS(OS os, string os_ver);
		int SetApps(int apps, float used_rom);
